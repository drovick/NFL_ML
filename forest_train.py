...
def multitree_get_tree(index,importance_list,error_list,tree_list,n_estimators_list,maxdep_list,minsamp_split_list,minsamp_leaf_list,minweight_frac_leaf_list,maxfeat_list,maxleaf_nodes_list,minimp_dec_list,minimp_split_list):
  print('Model:',str(tree_list[indeex]),', n_estimators:', str(n_estimators_list[index]),', max_depth:', str(maxdep_list[index]), ', min_samples_split:', str(minsamp_split_list[index]))
  print('min_samples_leaf:', str(minsamp_leaf_list[index]), 'min_weight_fraction_leaf:', str(minweight_frac_leaf_list[index]), ', max_features:,', str(maxfeat_list[index]))
...
def multitree_fit_errors(model,train_in,train_out,test_in,test_out,input_cols_,output_cols_,n_estimators_=int(10),max_depth_=None,min_samples_split_=2,min_samples_leaf_=1,min_weight_fraction_leaf_=0,max_features_=None,max_leaf_nodes_=None,min_impurity_decrease_=0):
...
def multitree_loop_lin_results(models,train_in,train_out,test_in,test_out,input_cols_,output_cols_,n_estimators,max_depths,min_samples_splits,min_samples_leafs,min_weight_fraction_leafs,max_featuress,max_leaf_nodess,min_impurity_decreases):
  ...
  for mod in models:
    for n_estimators__ in n_estimators:
              ...
              for maxfeat in max_featuress:
                for maxleaf_nodes in max_leaf_nodess:
                  for minimp_dec in min_impurity_decreases:
                    ...
                    i +=1
                    importances,error = multitree_fit_errors(mod,train_in,train_out,test_in,test_out,input_cols,output_cols_,n_estimators_=n_estimators__,max_depth_=depth,min_samples_split_=minsamp_split,min_samples_leaf_=minsamp_leaf,min_weight_fraction_leaf_=minweight_frac,max_features_=maxfeat,max_leaf_nodes_=maxleaf_nodes,min_impurity_decrease_=minimp_dec)         
                    ...
                    print(str(n_estimators__),' ',str(depth),' ',str(minsamp_split),' ',str(minsamp_leaf),' ',str(minweight_frac),' ',str(maxfeat),' ',str(maxleaf_nodes),' ',str(minimp_dec))    
                    n_estimators_list.append(n_estimators__)
...
i_list,e_list,t_list,estimators,maxdeps,minsamps,minsamps1,minweights,maxfeats,maxleafs,minimp_decs = multitree_loop_lin_results([RandomForestRegressor],train_set_input_normalized,train_set_output,test_set_input_normalized,test_set_output,input_cols,output_cols,n_estimators=[int(10),int(2)],max_depths=[10,5],min_samples_splits=[5000,1000],min_samples_leafs=[1000,2000],min_weight_fraction_leafs=[0.5,0.1],max_featuress=[5,10],max_leaf_nodess=[5,10],min_impurity_decreases=[float(+0.01),float(+0.1)])

i,e,t,estimat,maxd,minsa,minsa1,minwei,maxfe,maxlea,minidecs = multitree_loop_lin_results([RandomForestRegressor],train_set_input_normalized,train_set_output,test_set_input_normalized,test_set_output,input_cols,output_cols,n_estimators=[int(5)],max_depths=[5],min_samples_splits=[2],min_samples_leafs=[1],min_weight_fraction_leafs=[0],max_featuress=[75],max_leaf_nodess=[None],min_impurity_decreases=[float(+0.01)])
...
import pickle
filename = 'randomforest_pickle_test'
