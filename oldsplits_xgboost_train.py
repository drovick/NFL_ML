...
filename = 'oldsplits_xgb_pickle_2'
outfile = open(filename,'wb')
pickle_objs = [i_list,e_list,t_list,iters,subs,rates,estimators,maxdeps,minsamps,maxfeats,minimp_decs]
...
i,e,t,it,su,ra,estimat,maxd,minsa,maxfe,minidecs = gbr_multitree_loop_lin_results(['XGBRegressor'],train_set_input_normalized,train_set_output,val_set_input_normalized,val_set_output,input_cols,output_cols,n_iter_no_change__=+10,subsample=[float(+0.1),float(+0.01),float(+0.005)],learning_rate=[float(0.01),float(0.001),float(0.005)],n_estimators=[80],max_depths=[30,40,50,60],min_samples_leafs=[2],max_featuress=[0.5,0.4,0.3,0.2],min_impurity_decreases=[float(+0.001),float(+0.005)])
