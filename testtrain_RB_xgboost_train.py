...
full_set = pd.DataFrame(dataset[dataset.RB==1])
...
import pickle
filename = '_lessfit_normal_oldsplits_xgb_pickle_1'
...
filename = '_smallbatch_testtrain_oldsplits_xgb_pickle_2'
outfile = open(filename,'wb')
pickle_objs = [i_list,e_list,t_list,iters,subs,rates,estimators,maxdeps,minsamps,maxfeats,minimp_decs]

for obj in pickle_objs:
  pi  ckle.dump(obj,outfile)

outfile.close()

print('pickling complete, training the final group..')

i,e,t,it,su,ra,estimat,maxd,minsa,maxfe,minidecs = gbr_multitree_loop_lin_results(['XGBRegressor'],train_set_input_normalized,train_set_output,train_set_input_normalized,train_set_output,input_cols,output_cols,n_iter_no_change__=+5,subsample=[float(+0.01),float(+0.005),float(+0.0001)],learning_rate=[float(0.001),float(0.001),float(0.00001)],n_estimators=[100],max_depths=[50,80],min_samples_leafs=[2],max_featuress=[0.35,0.25,0.15],min_impurity_decreases=[float(0.005)])
...
filename = '_lessfit_testtrain_oldsplits_xgb_pickle'
outfile = open(filename,'wb')
