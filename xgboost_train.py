...
X_train, X_test, y_train, y_test = sk_learn.model_selection.train_test_split(full_set_input, full_set_output, test_size=0.2)
X_train, X_val, y_train, y_val = sk_learn.model_selection.train_test_split(X_train, y_train, test_size=0.2)
                                ...
                                for maxfeat in max_featuress:
                                    for minimp_dec in min_impurity_decreases:
                                        ...
                                        for col in output_cols_:
                                            #c+=1
                                            importances,error = gbr_multitree_fit_errors(mod,train_in,train_out[col],test_in,test_out[col],input_cols,col,n_iter_no_change_=iters,subsample_=sub,learning_rate_=rate,n_estimators_=n_estimators,max_depth_=depth,min_samples_leaf_=minsamp_leaf,max_features_=maxfeat,min_impurity_decrease_=minimp_dec)               
                                            #if c == 10:
...
tmp = time.time()
i_list,e_list,t_list,iters,subs,rates,estimators,maxdeps,minsamps,maxfeats,minimp_decs = gbr_multitree_loop_lin_results(['XGBRegressor'],train_set_input_normalized,train_set_output,val_set_input_normalized,val_set_output,input_cols,output_cols,n_iter_no_change__=[+int(80)],subsample=[float(+1.0)],learning_rate=[float(+0.1)],n_estimators=[100,400],max_depths=[30,90],min_samples_leafs=[1],max_featuress=[0.05,0.1,0.2],min_impurity_decreases=[float(+0.01),float(+0.001),float(+0.0005)])
